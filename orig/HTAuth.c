...
PRIVATE HTAAUser *decompose_auth_string ARGS2(char *, authstring, HTAAScheme, scheme)
{
    ...
    */
    ...
    {
        CTRACE(stderr, "ERROR....... Password missing from auth.string\n");
        return NULL;
    }
    *(password++) = '\0';
...
{
    if (HTAA_UNKNOWN == req->scheme || !HTProt || -1 == HTList_indexOf(HTProt->valid_schemes, (void *)req->scheme))
        return NO;
    ...
    case HTAA_BASIC:
    case HTAA_PUBKEY: {
        HTAAUser *user = decompose_auth_string(HTAuthString, req->scheme);
        /* Remember, user is auto-freed */
        if (user && HTAA_checkPassword(user->username, user->password, HTAssocList_lookup(HTProt->values, "passw")) &&
