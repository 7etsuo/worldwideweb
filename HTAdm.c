...
PRIVATE void usage NOARGS
{
    fprintf(stderr, "\n%s\n\nUsage:\n\t%s\n\t%s\n\t%s\n\t%s\n\t%s\n\n",
            ...
            "htadm -adduser pwfile [user [password  [real name & other stuff...]]]", "htadm -deluser pwfile [user]",
            "htadm -passwd  pwfile [user [password]]", "htadm -check   pwfile [user [password]]",
            "htadm -create  pwfile");
    exit(1);
}
...
PUBLIC int main ARGS2(int, argc, char **, argv)
{
    ...
    case HTADM_CREATE:
        ...
        {
            fprintf(stderr, "htadm:  Error: File \"%s\" already exists\n", argv[2]);
            fclose(fin);
            exit(10);
        }
        else if (!(fout = fopen(argv[2], "w")))
        {
            fprintf(stderr, "htadm:  Error: Couldn't create file \"%s\"\n", argv[2]);
            exit(11);
        }
        else
        {
    ...
    case HTADM_ADDUSER:
        ...
        {
            fprintf(stderr, "htadm:  Error: Can't stat password file \"%s\"\n", argv[2]);
            exit(2);
        }

        if (!(fin = fopen(argv[2], "r")))
        {
            fprintf(stderr, "htadm:  Error: Unable to open password file \"%s\"\n", argv[2]);
            exit(2);
        }
...
#ifndef VMS
        umask(0);
        fd = open(filename, O_WRONLY | O_CREAT | O_TRUNC, (S_IRWXU | S_IRWXG | S_IRWXO) & stat_in.st_mode);
        ...
        {
#endif
            fprintf(stderr, "htadm:  Error: Unable to create temporary file \"%s\"\n", filename);
            fclose(fin);
            exit(3);
        }
        ...
        if ((found && oper == HTADM_ADDUSER) || (!found && (oper == HTADM_PASSWD || oper == HTADM_DELUSER)))
        {
            fprintf(stderr, "htadm:  Error: Entry for user \"%s\" %s in password file\n", username,
        ...
        if (oper == HTADM_ADDUSER || oper == HTADM_PASSWD)
        {
            ...
            {
                if (argc > 5)
                { /* Real name given in command line */
        ...
        {
            fprintf(stderr, "%s %d %s %d\n%s\n", "htadm:  Error: new file size", (int)stat_out.st_size,
                    "doesn't match expected", (int)(stat_in.st_size + sizediff),
