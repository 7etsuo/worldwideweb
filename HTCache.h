...
*/
PUBLIC HTCacheTask cache_lookup PARAMS((char *url, char **cfn, FILE **cf, time_t *if_ms, time_t *expires));

PUBLIC HTStream *HTProxyCache PARAMS((HTStream * output_stream, FILE *cf, char *cfn, char *url, time_t if_ms));
...
PUBLIC void HTLoadCacheToStream PARAMS((FILE * fp, HTStream *sink, time_t expires));

PUBLIC FILE *HTCacheInfo_open PARAMS((char *dirname, char *mode));

PUBLIC FILE *HTCacheInfo_openFor PARAMS((char *filename, char *mode));

PUBLIC void HTCacheInfo_remove PARAMS((char *dirname));

PUBLIC BOOL HTCacheInfo_next PARAMS((FILE * fp, char **filename_p, time_t *load_delay, time_t *last_checked,
                                     time_t *expires, time_t *max_unused, time_t *last_modified));

PUBLIC BOOL HTCacheInfo_for PARAMS((char *filename, time_t *load_delay, time_t *last_checked, time_t *expires,
                                    time_t *max_unused, time_t *last_modified));

PUBLIC void HTCacheInfo_writeEntry PARAMS((FILE * fp, char *filename, time_t load_delay, time_t last_checked,
                                           time_t expires, time_t max_unused, time_t last_modified));

PUBLIC BOOL HTCacheInfo_writeEntryFor PARAMS((char *filename, time_t delay, time_t last_chk, time_t exp,
                                              time_t max_unused, time_t lm));
